### Предистория: 
Решал я таски на [rootme](https://www.root-me.org/) и попалась таска [XMPP - authentication](https://www.root-me.org/en/Challenges/Network/XMPP-authentication-197). Основная цель таски состояла в том, чтобы по захвату пакетов вытащить пароль, который использовался при аутентификации и я начал искать документацию к тому, как работает аутентификация клиента.
...
У XMPP есть 2 способа аутентификации клиента SASL[^1] и OAuth 2.0. Начнем по порядку - с механизмов аутентификации SASL, используемые в XMPP. 
...
Как оно работает?
1. Клиент отправляет имя пользователя, под которым он хочет аутентифицироваться
